...
def upload():
    ...
    if isinstance(data, list):
        records.extend(data)
        print(f"[Server] Received {len(data)} records (total stored: {len(records)})")
        return jsonify({"status":"ok","received":len(data)}), 200
    elif isinstance(data, dict):
        records.append(data)
        print(f"[Server] Received 1 record (total stored: {len(records)})")
...
def get_data():
    return jsonify(records[-500:]), 200
...
def get_labs():
    ...
    for r in records:
        ...
        lab_map.setdefault(lab,set()).add(sys)
