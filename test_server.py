...
MONGO_URI = os.getenv(
    "MONGO_URI",
    "mongodb+srv://dhayalan:dhayalan@05@cluster0.mongodb.net/?retryWrites=true&w=majority"
)
...
def train_model():
    ...
    if not rows:
        print("⚠️ No records found for training.")
        return
    ...
    print(f"✅ ML retrained on {len(urls)} samples.")
...
def predict_category(url):
    try:
        if not (os.path.exists(CAT_MODEL_FILE) and os.path.exists(CAT_VECT_FILE)):
...
def insert_records(records):
    """Insert browsing records in MongoDB."""
    if not records:
        return
    docs = []
    for r in records:
        ...
        docs.append({
            ...
            "flagged": r.get("flagged", 0) or predict_flag(url)
        })
    if docs:
...
def upload():
    """Receives JSON data from agent and inserts into MongoDB."""
    try:
...
def train_from_csv(csv_path):
    ...
    if 'url' not in df.columns or 'category' not in df.columns:
        print("❌ CSV must contain 'url' and 'category'")
        return
    ...
    joblib.dump(cat_model, CAT_MODEL_FILE)
    joblib.dump(cat_vectorizer, CAT_VECT_FILE)
    print(f"✅ Category model trained on {len(urls)} samples from CSV.")
