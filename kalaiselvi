...
def main_gui():
    ...
    output_box = scrolledtext.ScrolledText(root, wrap=tk.WORD, width=110, height=32)
    ...
    if not os.path.exists(src_path):
        output_box.insert(tk.END, "❌ Chrome history not found. Exiting.\n")
        root.mainloop()
        return
    ...
    if ml_report:
        output_box.insert(tk.END, "🔹 ML Classification Report:\n")
        output_box.insert(tk.END, ml_report + "\n")
